...
import torch.hub
from dataset import load_segment
from PIL import Image, ImageOps
...
if __name__ == "__main__":
    ...
    segment_count = 8
    base_model = 'resnet50'
    net = torch.hub.load(repo, 'TSN', class_counts, segment_count, 'RGB',
    ...
    # img = Image.open('data/frame_0.jpg').convert('RGB')
    # rst = transform([img] * segment_count)
    
    frames = load_segment('data/segment_140.mp4')
    rst = transform(frames)
    rst = np.array(rst)
    print("Max val: ", np.max(rst))
    print(rst.shape)
    print(rst[0, :,:].shape)
    print(np.mean(rst[0, :, :]))
    quit(0)
